...
Import.mediaImage = function(key, productImage){

   const productId = productImage.productId;

   const variantId = productImage.variantId;
   const shopId = productImage.shopId;
   const userId = productImage.ownerId;
   const imageUrl = productImage.imageUrl;
   let count = Collections.Media.find({
     "metadata.variantId": variantId
   }).count();

let fileObj = new FS.File();
fileObj.attachData("C:/order/reaction/public/resources/products/"+imageUrl, function (error) {
   error && console.log(error);
   fileObj._id=productImage._id;
   fileObj.metadata = {
     ownerId: userId,
     productId: productId,
     variantId: variantId,
   ...
   console.log("fileObj");
   console.log(fileObj);
   Collections.Media.insert(fileObj);
 });
}
...
Import.indication("packName", Collections.Prepacks, 0.5);
