...
export default {
  ...
  getVariantPriceRange(variantId) {
     ...
     * @return {String} formatted price or price range
     */
    getVariantEstimatedFCCRange(variantId) {
      const children = this.getVariants(variantId);
      const visibleChildren = children.filter(child => child.isVisible);
      ...
      switch (visibleChildren.length) {
        case 0:
          ...
          console.log(topVariant);
          console.log(topVariant.estimatedFCC);
          // topVariant could be undefined when we removing last top variant
          return topVariant && topVariant.estimatedFCC;
        case 1:
        console.log("visibleChildren[0].estimatedFCC");
        console.log(visibleChildren[0].estimatedFCC);
          return visibleChildren[0].estimatedFCC;
        default:
          let priceMin = Number.POSITIVE_INFINITY;
          let priceMax = Number.NEGATIVE_INFINITY;
          ...
          children.map(child => {
            if (child.isVisible === true) {
              if (child.estimatedFCC < priceMin) {
                priceMin = child.estimatedFCC;
              }
              if (child.estimatedFCC > priceMax) {
                priceMax = child.estimatedFCC;
              }
            }
          });
